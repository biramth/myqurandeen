...
export const hadithApi = {
  ...
  getBookHadiths: (slug: string, bookNumber: number, page = 1, lang?: string) =>
    apiClient.get<HadithBookPage>(
      `/hadith/collections/${slug}/books/${bookNumber}?page=${page}${lang ? `&lang=${lang}` : ""}`,
      { skipAuth: true },
    ),
  getHadith: (slug: string, number: string, lang?: string) =>
    apiClient.get<HadithDetail>(
      `/hadith/collections/${slug}/hadiths/${number}${lang ? `?lang=${lang}` : ""}`,
      { skipAuth: true },
    ),
